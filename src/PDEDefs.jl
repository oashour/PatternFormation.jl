    ...
    @floop ex for j in 1:N, i in 1:N
      @inbounds begin
        ...
        elseif i == 2 && j != N && j != 1 && j != N-1 && j != 2# left sub-boundary
          du[i,j,1] =  -γᵤ*Mᵤ*(1/dx^4*(u[i+2,j,1] - 4u[i+1,j,1] + 6u[i,j,1] - 4u[i-1,j,1] + u[N, j, 1]) +
                             1/dy^4*(u[i,j+2,1] - 4u[i,j+1,1] + 6u[i,j,1] - 4u[i,j-1,1] + u[i, j-2, 1])) +
                      ...
                      + c - a*u[i,j,1]*u[i,j,2]
          du[i,j,2] =  -γᵥ*Mᵥ*(1/dx^4*(u[i+2,j,2] - 4u[i+1,j,2] + 6u[i,j,2] - 4u[i-1,j,2] + u[N, j, 2]) +
                             1/dy^4*(u[i,j+2,2] - 4u[i,j+1,2] + 6u[i,j,2] - 4u[i,j-1,2] + u[i, j-2, 2])) +
        ...
        elseif i == N-1 && j != N && j != 1 && j != N-1 && j != 2 # right sub boundary
          du[i,j,1] =  -γᵤ*Mᵤ*(1/dx^4*(u[1,j,1] - 4u[i+1,j,1] + 6u[i,j,1] - 4u[i-1,j,1] + u[i-2, j, 1]) +
                             1/dy^4*(u[i,j+2,1] - 4u[i,j+1,1] + 6u[i,j,1] - 4u[i,j-1,1] + u[i, j-2, 1])) +
                      2*Mᵤ*(6u[i,j,1]^2 - 6u[i,j,1] + 1)*(1/dx^2*(u[i+1,j,1] + u[i-1,j,1] - 2u[i,j,1]) +
                                                          1/dy^2*(u[i,j+1,1] + u[i,j-1,1] - 2u[i,j,1])) +
                      12*Mᵤ*(2u[i,j,1] - 1)*(1/2/dx*(u[i+1,j,1]-u[i-1,j,1]) + 1/2/dy*(u[i,j+1,1]-u[i,j-1,1]))^2
                      + c - a*u[i,j,1]*u[i,j,2]
          du[i,j,2] =  -γᵥ*Mᵥ*(1/dx^4*(u[1,j,2] - 4u[i+1,j,2] + 6u[i,j,2] - 4u[i-1,j,2] + u[i-2, j, 2]) +
                             1/dy^4*(u[i,j+2,2] - 4u[i,j+1,2] + 6u[i,j,2] - 4u[i,j-1,2] + u[i, j-2, 2])) +
                      2*Mᵥ*(6u[i,j,2]^2 - 6u[i,j,2] + 1)*(1/dx^2*(u[i+1,j,2] + u[i-1,j,2] - 2u[i,j,2]) +
                                                          1/dy^2*(u[i,j+1,2] + u[i,j-1,2] - 2u[i,j,2])) +
                      12*Mᵥ*(2u[i,j,2] - 1)*(1/2/dx*(u[i+1,j,2]-u[i-1,j,2]) + 1/2/dy*(u[i,j+1,2]-u[i,j-1,2]))^2
        ...
        elseif j == 2 && i != N && i != 1 && i != N-1 && i != 2# bottom sub boundary
          du[i,j,1] =  -γᵤ*Mᵤ*(1/dx^4*(u[i+2,j,1] - 4u[i+1,j,1] + 6u[i,j,1] - 4u[i-1,j,1] + u[i-2, j, 1]) +
                             1/dy^4*(u[i,j+2,1] - 4u[i,j+1,1] + 6u[i,j,1] - 4u[i,j-1,1] + u[i, N, 1])) +
                      2*Mᵤ*(6u[i,j,1]^2 - 6u[i,j,1] + 1)*(1/dx^2*(u[i+1,j,1] + u[i-1,j,1] - 2u[i,j,1]) +
                                                          1/dy^2*(u[i,j+1,1] + u[i,j-1,1] - 2u[i,j,1])) +
                      12*Mᵤ*(2u[i,j,1] - 1)*(1/2/dx*(u[i+1,j,1]-u[i-1,j,1]) + 1/2/dy*(u[i,j+1,1]-u[i,j-1,1]))^2
                      + c - a*u[i,j,1]*u[i,j,2]
          du[i,j,2] =  -γᵥ*Mᵥ*(1/dx^4*(u[i+2,j,2] - 4u[i+1,j,2] + 6u[i,j,2] - 4u[i-1,j,2] + u[i-2, j, 2]) +
                             1/dy^4*(u[i,j+2,2] - 4u[i,j+1,2] + 6u[i,j,2] - 4u[i,j-1,2] + u[i, N, 2])) +
                      2*Mᵥ*(6u[i,j,2]^2 - 6u[i,j,2] + 1)*(1/dx^2*(u[i+1,j,2] + u[i-1,j,2] - 2u[i,j,2]) +
                                                          1/dy^2*(u[i,j+1,2] + u[i,j-1,2] - 2u[i,j,2])) +
                      12*Mᵥ*(2u[i,j,2] - 1)*(1/2/dx*(u[i+1,j,2]-u[i-1,j,2]) + 1/2/dy*(u[i,j+1,2]-u[i,j-1,2]))^2
                      + c - a*u[i,j,1]*u[i,j,2]
        ...
        elseif j == N && i != N && i != 1 && i != N-1 && i != 2 # top sub boundary
          du[i,j,1] =  -γᵤ*Mᵤ*(1/dx^4*(u[i+2,j,1] - 4u[i+1,j,1] + 6u[i,j,1] - 4u[i-1,j,1] + u[i-2, j, 1]) +
                      ...
                      + c - a*u[i,j,1]*u[i,j,2]
          du[i,j,2] =  -γᵥ*Mᵥ*(1/dx^4*(u[i+2,j,2] - 4u[i+1,j,2] + 6u[i,j,2] - 4u[i-1,j,2] + u[i-2, j, 2]) +
                             1/dy^4*(u[i,2,2] - 4u[i,j+1,2] + 6u[i,j,2] - 4u[i,j-1,2] + u[i, j-2, 2])) +
                      2*Mᵥ*(6u[i,j,2]^2 - 6u[i,j,2] + 1)*(1/dx^2*(u[i+1,j,2] + u[i-1,j,2] - 2u[i,j,2]) +
                                                          1/dy^2*(u[i,j+1,2] + u[i,j-1,2] - 2u[i,j,2])) +
                      12*Mᵥ*(2u[i,j,2] - 1)*(1/2/dx*(u[i+1,j,2]-u[i-1,j,2]) + 1/2/dy*(u[i,j+1,2]-u[i,j-1,2]))^2
                      + c - a*u[i,j,1]*u[i,j,2]
        ...
        elseif i == 2 && j == N-1 # left top subcorner
          du[i,j,1] =  -γᵤ*Mᵤ*(1/dx^4*(u[i+2,j,1] - 4u[i+1,j,1] + 6u[i,j,1] - 4u[i-1,j,1] + u[N, j, 1]) +
                             1/dy^4*(u[i,1,1] - 4u[i,j+1,1] + 6u[i,j,1] - 4u[i,j-1,1] + u[i, j-2, 1])) +
                      2*Mᵤ*(6u[i,j,1]^2 - 6u[i,j,1] + 1)*(1/dx^2*(u[i+1,j,1] + u[i-1,j,1] - 2u[i,j,1]) +
                                                          1/dy^2*(u[i,j+1,1] + u[i,j-1,1] - 2u[i,j,1])) +
                      12*Mᵤ*(2u[i,j,1] - 1)*(1/2/dx*(u[i+1,j,1]-u[i-1,j,1]) + 1/2/dy*(u[i,j+1,1]-u[i,j-1,1]))^2
                      + c - a*u[i,j,1]*u[i,j,2]
          du[i,j,2] =  -γᵥ*Mᵥ*(1/dx^4*(u[i+2,j,2] - 4u[i+1,j,2] + 6u[i,j,2] - 4u[i-1,j,2] + u[N, j, 2]) +
                             1/dy^4*(u[i,1,2] - 4u[i,j+1,2] + 6u[i,j,2] - 4u[i,j-1,2] + u[i, j-2, 2])) +
                      2*Mᵥ*(6u[i,j,2]^2 - 6u[i,j,2] + 1)*(1/dx^2*(u[i+1,j,2] + u[i-1,j,2] - 2u[i,j,2]) +
                                                          1/dy^2*(u[i,j+1,2] + u[i,j-1,2] - 2u[i,j,2])) +
                      12*Mᵥ*(2u[i,j,2] - 1)*(1/2/dx*(u[i+1,j,2]-u[i-1,j,2]) + 1/2/dy*(u[i,j+1,2]-u[i,j-1,2]))^2
                      + c - a*u[i,j,1]*u[i,j,2]
        elseif i == N && j == 1 # right bottom corner
          du[i,j,1] =  -γᵤ*Mᵤ*(1/dx^4*(u[2,j,1] - 4u[1,j,1] + 6u[i,j,1] - 4u[i-1,j,1] + u[i-2, j, 1]) +
                             1/dy^4*(u[i,j+2,1] - 4u[i,j+1,1] + 6u[i,j,1] - 4u[i,N,1] + u[i, N-1, 1])) +
                      2*Mᵤ*(6u[i,j,1]^2 - 6u[i,j,1] + 1)*(1/dx^2*(u[1,j,1] + u[i-1,j,1] - 2u[i,j,1]) +
                                                          1/dy^2*(u[i,j+1,1] + u[i,1,1] - 2u[i,j,1])) +
        ...
        elseif i == 2 && j == N
          du[i,j,1] =  -γᵤ*Mᵤ*(1/dx^4*(u[i+2,j,1] - 4u[i+1,j,1] + 6u[i,j,1] - 4u[i-1,j,1] + u[N, j, 1]) +
                             1/dy^4*(u[i,2,1] - 4u[i,1,1] + 6u[i,j,1] - 4u[i,j-1,1] + u[i, j-2, 1])) +
                      2*Mᵤ*(6u[i,j,1]^2 - 6u[i,j,1] + 1)*(1/dx^2*(u[i+1,j,1] + u[i-1,j,1] - 2u[i,j,1]) +
                                                          1/dy^2*(u[i,j,1] + u[i,j-1,1] - 2u[i,j,1])) +
                      12*Mᵤ*(2u[i,j,1] - 1)*(1/2/dx*(u[i+1,j,1]-u[i-1,j,1]) + 1/2/dy*(u[i,j,1]-u[i,j-1,1]))^2
                      + c - a*u[i,j,1]*u[i,j,2]
          du[i,j,2] =  -γᵥ*Mᵥ*(1/dx^4*(u[i+2,j,2] - 4u[i+1,j,2] + 6u[i,j,2] - 4u[i-1,j,2] + u[N, j, 2]) +
                             1/dy^4*(u[i,2,2] - 4u[i,1,2] + 6u[i,j,2] - 4u[i,j-1,2] + u[i, j-2, 2])) +
                      2*Mᵥ*(6u[i,j,2]^2 - 6u[i,j,2] + 1)*(1/dx^2*(u[i+1,j,2] + u[i-1,j,2] - 2u[i,j,2]) +
